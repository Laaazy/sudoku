...
//得到所有答案
void DLXSolver_1::solveWithAllAnswers(DLXNode_1 *listHead, vector<CommonNode_1*>& tempSolution, vector<vector<CommonNode*>>& lastSolution, int depth) {
	if (listHead->rightNode == listHead) { //找到一个解
...
//得到某一数量的解
void DLXSolver_1::solveWithCertainAnswers(DLXNode_1 *listHead, vector<CommonNode_1*>& tempSolution, vector<vector<CommonNode*>>& lastSolution,
...
ColumnHead_1* DLXSolver_1::selectColumn(DLXNode_1* listHead) {
	...
	for (DLXNode_1* node = listHead->rightNode; node != listHead; node = node->rightNode) {
		ColumnHead_1* columnHead = (ColumnHead_1*)node;
